...
# --- Gauge using tkinter Canvas (no matplotlib, drawn once) ---
class GaugeCanvas(ctk.CTkFrame):
    def __init__(self, parent, title, color, **kwargs):
        ...
        self.canvas = ctk.CTkCanvas(self, bg="#161b22", highlightthickness=0,
        ...
        # Draw static background arc once
...
class AIBootDashboard(ctk.CTk):
    def __init__(self):
        ...
        # Load models in background, then start refresh
    ...
    def _preload(self):
        ...
        try:
            if os.path.exists(ANOMALY_MODEL):
                self._anomaly_model = joblib.load(ANOMALY_MODEL)
        ...
        # Trigger retrain check in background
        try:
        ...
        # Start refresh loop after preload
    ...
    def build_ui(self):
        ...
        # Gauges — drawn once, updated by value only
        ...
        # Graph — matplotlib, redraws only on new data
    ...
    def _fetch_data(self):
        ...
        # Boot prediction — model already loaded
        try:
        ...
        # Threat detection — model already loaded
        try:
            if self._anomaly_model and os.path.exists(THREAT_FILE):
                from ml_model import detect_threats, build_process_profile, is_legitimate
        ...
        try:
            ...
            if self._driver_cache is None or (now - self._driver_last_scan) > 300:
                from driver_analyzer import analyze_drivers
    ...
    def _update_ui(self, data):
        try:
            if 'health_score' in data:
                ...
                # Gauges — only update value, no full redraw
